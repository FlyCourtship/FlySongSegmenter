...
c= round(Fs/2500); %factor for computing window around pulse peak (this determines how much of the signal before and after the peak is included in the pulse, and sets the parameters w0 and w1.)
d = 4; %factor times the mean of xempty - only pulses larger than this amplitude are counted as true pulses

e = 8; % pwr: Power to raise signal by
f = round(Fs/500)+1; %pWid:  Approx Pulse Width in points (odd, rounded)
g = round(Fs/80); % buff: Points to take around each pulse for finding pulse peaks
h = round(Fs/50); %lowIPI: estimate of a very low IPI (even, rounded)
...
k = round(Fs/2); %if no other pulse within this many samples, do not count as a pulse (the idea is that a single pulse, not within IPI range of another pulse, is likely not a true pulse)
l = 700; %if best matched scale is greater than this frequency, then don't include pulse as true pulse
m = round(Fs/40); %if pulse peaks are this close together, only keep the larger pulse (this value should be less than the species-typical IPI)
