...
for i in $(seq $nchan)
do
  qsub -N "FSS-$clean_daq_name-$clean_params_name-$i" -pe batch 1 -l short=true -b y -j y -cwd -o "$daq_name-$params_name-$i.log" -V ./find_fly_song/distrib/run_find_fly_song.sh /usr/local/matlab-2012a "\"$daq_path\"" -p "\"$params_path\"" -c "$i"
done
