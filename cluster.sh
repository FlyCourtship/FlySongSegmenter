...
#each channel of each file is sent to it's own whole node.  all files assumed to have the
...
do
  ...
  for i in $(seq $nchan)
  do
    qsub -N "FSS-$clean_daq_name-$i-$clean_params_name" -pe batch 8 -l short=true -b y -j y -cwd -o "$daq_folder/$daq_file/$daq_name-$i-$params_name.log" -V ./find_fly_song/distrib/run_find_fly_song.sh /usr/local/matlab-2012a "\"$daq_file\"" -p "\"$params_path\"" -c "$i" -s 1:1000000
  done
done
