...
do
  ...
  do
    ...
    qsub -N FSS-$clean_daq_name-$i-$clean_params_name -l short=true -b y -j y -cwd -o $daq_folder/$daq_name\_out/$clean_daq_name-$i-$clean_params_name.log -V ./cluster2.sh "\"$daq_file\"" -p "\"$params_path\"" -c "$i"
    #sleep 1
  done
done
